...
LEFT JOIN Trained_In T ON P.EmployeeID = T.Physician AND U.Procedure = T.Treatment
...
LEFT JOIN Trained_In t on t.Physician = u.Physician and t.Treatment = u.Procedure
WHERE t.Physician not null and u.date > t.CertificationExpires or u.date < t.CertificationDate
...
SELECT p.name, pr.name, u.date, pat.name, t.CertificationExpires
FROM Physician p
JOIN Undergoes u ON u.Physician = p.EmployeeID 
LEFT JOIN Trained_In t on t.Physician = u.Physician and t.Treatment = u.Procedure
left join Procedures pr on pr.Code = t.Treatment
LEFT JOIN Patient pat on pat.ssn = u.Patient
WHERE t.Physician not null and u.date > t.CertificationExpires or u.date < t.CertificationDate
...
SELECT p.name, py.name, a.PrepNurse, a.Start, a.End, a.ExaminationRoom, py2.Name
from Appointment a
left join patient p on p.SSN = a.Patient
join Physician py on py.EmployeeID = a.Physician
join Physician py2 on py2.EmployeeID = p.pcp
...
-- the Undergoes table may include a row where the patient ID does not match the one we would obtain from the Stay table through the Undergoes.Stay foreign key.
-- Select all rows from Undergoes that exhibit this inconsistency.

SELECT u.*
FROM Undergoes u
LEFT JOIN Stay s on s.Patient = u.Patient
...
SELECT pat.name, phy.name
from Prescribes pre
left join Patient pat ON pre.Patient = pat.ssn
left JOIN Physician phy on phy.EmployeeID = pre.Physician
...
SELECT pat.name, phy.name
from Undergoes u
left join Procedures p on p.code = u.Procedure
left join Patient pat ON u.Patient = pat.ssn
left JOIN Physician phy on phy.EmployeeID = u.Physician
where p.cost > 5000;
	
    -- The patient has had at least two appointments where the nurse who prepared the appointment was a registered nurse.
	
SELECT pat.name, phy.name, count (Registered) as occurance
from Appointment a
join nurse n on n.EmployeeID = a.PrepNurse
left join Patient pat ON a.Patient = pat.ssn
left JOIN Physician phy on phy.EmployeeID = a.Physician
GROUP by a.patient
HAVING n.Registered = 1 and occurance >=2;
...
select pat.name, phy.name
from patient pat
left join Department d on d.head = pat.PCP
LEFT join Physician phy on pat.pcp = phy.EmployeeID
